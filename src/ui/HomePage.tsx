...
const HomePage: React.FC<HomePageProps> = ({
  ...
  return (
    <>
      <HeroSection copy={copy} onWhatsappClick={onWhatsappClick} getWhatsappHref={getWhatsappHref} />
...
const HeroSection: React.FC<{ copy: ReturnType<typeof getLandingCopy>; onWhatsappClick: (label: string) => void; getWhatsappHref: (label: string) => string; }> = ({ copy, onWhatsappClick, getWhatsappHref }) => (
    ...
    <div className="home-hero__grid relative mx-auto max-w-8xl">
      <div className="home-hero__mobile-portrait">
        <div className="hero-image-layer">
          <div className="hero-image-wrapper">
            <img src={LINK_BIO_HERO.photo} alt={LINK_BIO_PROFILE.name} className="hero-image" loading="lazy" />
          </div>
        </div>
      </div>
      ...
      <div className="home-hero__content">
        <GlassCard className="home-hero__card w-full border-white/60 px-8 py-10 text-left">
          ...
          <h1 className="mt-4 text-3xl font-bold leading-tight text-white md:text-4xl">{copy.hero.title}</h1>
          <p className="mt-4 text-lg text-white/90 md:text-xl">{copy.hero.subtitle}</p>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <CTAButton label={copy.hero.ctaPrimary} href={getWhatsappHref(copy.hero.ctaPrimary)} />
            <button
              className="rounded-full border border-white/70 px-5 py-3 text-sm font-semibold text-white/90 transition hover:bg-white/10"
              onClick={() => onWhatsappClick(copy.hero.ctaSecondary)}
            >
...
const DiagnosticSection: React.FC<{ copy: ReturnType<typeof getLandingCopy> }> = ({ copy }) => {
  ...
  const totalSymptoms = copy.diagnostic.symptoms.length;

  return (
    <SectionWave className="bg-gradient-to-br from-surface-100 via-blush-300/30 to-surface-100">
      <div className="mx-auto max-w-6xl px-6">
        <SectionTitle label={copy.diagnostic.title} />
        <GlassCard className="p-6 md:p-8">
          <div className="grid gap-4 sm:grid-cols-2">
            {copy.diagnostic.symptoms.map((item) => {
              ...
              return (
                ...
                >
                  <span className={`icon-circle text-lg transition-colors ${isSelected ? "bg-[var(--deep-forest)] text-white shadow-md" : ""}`}>✓</span>
                  <p className={`card-text text-base transition-colors ${isSelected ? "font-semibold text-[var(--deep-forest)]" : ""}`}>{item}</p>
                </button>
              );
            })}
          </div>
...
const AboutSection: React.FC<{ copy: ReturnType<typeof getLandingCopy>; onWhatsappClick: (label: string) => void }> = ({ copy, onWhatsappClick }) => {
        ...
        <div className="grid gap-8 md:grid-cols-[minmax(0,1.1fr)_minmax(0,1fr)] md:items-center">
          <div className="h-64 overflow-hidden rounded-3xl shadow-2xl md:h-80 lg:h-96">
            <img
              ...
              style={{ objectPosition: aboutMobilePosition, ["--about-desktop-position" as string]: aboutDesktopPosition }}
            />
          </div>
          <GlassCard className="p-6 md:p-8">
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <CTAButton label={copy.about.primaryCtaLabel} onClick={() => onWhatsappClick(copy.about.primaryCtaLabel)} />
              <button
                ...
                onClick={() => onWhatsappClick(copy.about.secondaryCtaLabel)}
              >
              ...
              <div className="flex flex-col gap-3">
                ...
                <ul className="space-y-2 text-sm text-neutral-900">
                  {program.bullets.map((bullet) => (
                    <li key={bullet} className="flex items-center gap-2"><span className="text-primary-700">✓</span><span>{bullet}</span></li>
                  ))}
                </ul>
                <div className="pt-2">
                  <CTAButton
                    label={program.cta}
                    href={isWhatsapp ? whatsappHref : goToCalculator ? calculatorHref : isReset ? resetHref : undefined}
                    onClick={
                      isWhatsapp
    ...
    <div className="mx-auto max-w-5xl px-6">
      <GlassCard className="p-6 md:p-10 text-center">
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-3">
          {story.bullets.map((item) => (<div key={item} className="glass-card p-4 text-sm font-semibold text-neutral-900">{item}</div>))}
        </div>
        <p className="mt-6 text-base text-neutral-600">{story.text}</p>
        <div className="mt-8 flex justify-center"><CTAButton label="Quero viver isso" onClick={() => onWhatsappClick("Quero viver isso")} /></div>
      </GlassCard>
    </div>
  </SectionWave>
);
          ...
          <GlassCard key={item.name} className="min-w-[260px] max-w-sm flex-1 p-5">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:gap-5">
              <div className="relative inline-flex h-14 w-14 shrink-0 items-center justify-center rounded-full bg-gradient-to-tr from-primary-300 via-peach-500/80 to-blush-300 p-[2px] sm:h-16 sm:w-16">
                <img src={item.image} alt={item.name} className="h-full w-full rounded-full object-cover bg-surface-100" />
              </div>
              <div className="flex-1 space-y-3">
                ...
                <div className="inline-flex rounded-full bg-surface-100/60 px-3 py-1 text-xs font-semibold text-neutral-900">{item.tag}</div>
              </div>
            </div>
          </GlassCard>
        ))}
      ...
      <div className="mx-auto max-w-5xl px-6">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {faq.map((item, index) => {
            ...
            return (
              <div key={item.q} className="rounded-xl border border-white/70 bg-white/70 p-3 shadow-sm backdrop-blur">
                <button
                  type="button"
                ...
                >
                  ...
                  <span className="flex h-9 w-9 items-center justify-center rounded-full bg-surface-100 text-neutral-900 shadow-inner">
                    <svg className={`h-5 w-5 transition-transform duration-200 ${isOpen ? "rotate-180" : ""}`} viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path d="M5.5 7.5L10 12l4.5-4.5" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                  </span>
                </button>
                <div id={contentId} role="region" className={`overflow-hidden text-sm text-neutral-600 transition-all duration-300 ease-in-out ${isOpen ? "mt-2 max-h-40 opacity-100" : "max-h-0 opacity-0"}`}>
...
const FinalCTASection: React.FC<{ copy: ReturnType<typeof getLandingCopy>; onWhatsappClick: (label: string) => void }> = ({ copy, onWhatsappClick }) => (
  <section className="relative overflow-hidden bg-gradient-to-br from-primary-700/70 via-peach-500/60 to-blush-300/60 text-white">
    ...
    <div className="relative mx-auto flex max-w-5xl flex-col items-center justify-center px-6 py-16 text-center">
      <GlassCard className="max-w-3xl border-white/60 px-8 py-10 text-white">
        ...
        <p className="mt-4 text-lg text-white/90">{copy.finalCta.subtitle}</p>
        <div className="mt-6 flex flex-wrap justify-center gap-3">
          <CTAButton label={copy.finalCta.primaryLabel} onClick={() => onWhatsappClick(copy.finalCta.primaryLabel)} />
          <button
            className="rounded-full border border-white/70 px-5 py-3 text-sm font-semibold text-white/90 transition hover:bg-white/10"
            onClick={() => onWhatsappClick(copy.finalCta.secondaryLabel)}
          >
